...
import com.nb20_statistics.nb20_statistics.helper.DateHourMinHelper;
import com.nb20_statistics.nb20_statistics.utils.ScreenSize;
 ...
 */
public class StatisticsMonthWidget extends PercentRelativeLayout {

    private View inflate;
    private RecyclerView rcvStatistics;
    private Context context;
    private TextView tvMonthDes;
    ...
    public StatisticsMonthWidget(Context context) {
        this(context, null, 0);
    }

    public StatisticsMonthWidget(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public StatisticsMonthWidget(Context context, AttributeSet attrs, int defStyle) {
        ...
        findViewById(R.id.ll_widget_min_coodinary).setVisibility(INVISIBLE);// 分坐标
        findViewById(R.id.ll_widget_hour_coodinary).setVisibility(VISIBLE);// 时坐标
        tvMonthDes = findViewById(R.id.tv_widget_month);// 左下角月份描述(DAY WEEK无需显示)
        tvMonthDes.setVisibility(VISIBLE);
        // banner
        TextView tvExceriseMinHour = findViewById(R.id.tv_widget_excerise_min_hour);// Excerise / hours
        tvExceriseMinHour.setText(context.getString(R.string.banner_Excerisehours));
        // timer
    ...
    public void setStatistics(final List<StatisticsBean> beans) {
        rcvStatistics.post(new Runnable() {
            ...
            public void run() {
                ...
                StatisticsMonthAdapter adapter = new StatisticsMonthAdapter(context, beans, rcvStatistics);
                adapter.setOnClickItemListener(new StatisticsMonthAdapter.OnClickItemListener() {
                    @Override
                    public void clickItem(StatisticsBean statisticsBean) {
                        ...
                        int restTime = UNIT * 60 - playTime - walkTime;
     ...
     * @param month 月份
     */
    public void setMonthDes(@IntRange(from = 1, to = 12) int month) {
        List<String> monthEnglish = StatisticsUtils.getMonthEnglish(context);
        String monthText = monthEnglish.get(month - 1);
        tvMonthDes.setText(monthText);
    }

    /**
     * 计算目标线Y位移
     *
     * @param mins 0 ~ 1440 mins
     */
    public void setGoal(@IntRange(from = 0, to = UNIT * 60) final int mins) {
        ...
        // * 设置goal line文字显示
        String goalLineText = String.format(context.getString(R.string.goal_hours), mins / 60);
    ...
    private float getGoalCurrentMarginTop() {
        ...
        float recWidthPercent = Float.valueOf(context.getString(R.string.month_rec_widthPercent).split("%")[0]) / 100;
        float rlWidthPercent = Float.valueOf(context.getString(R.string.month_rlBlue_widthPercent).split("%")[0]) * 1f / 100;
    ...
    private float getGoalCurrentMarginBottom() {
        ...
        float tvSizePercent = Float.valueOf(context.getString(R.string.month_tv_sizePercent).split("%")[0]) / 100;
        float recWidthPercent = Float.valueOf(context.getString(R.string.month_rec_widthPercent).split("%")[0]) / 100;
        float rlWidthPercent = Float.valueOf(context.getString(R.string.month_rlBlue_widthPercent).split("%")[0]) * 1f / 100;
        float rlBlueMarginBottomPercent = Float.valueOf(context.getString(R.string.month_rlBlue_marginBottomPercent).split("%")[0]) * 1f / 100;
