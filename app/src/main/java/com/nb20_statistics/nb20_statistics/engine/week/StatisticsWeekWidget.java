package com.nb20_statistics.nb20_statistics.engine.week;
 ...
 */
public class StatisticsWeekWidget extends PercentRelativeLayout {
    ...
    public StatisticsWeekWidget(Context context) {
        this(context, null, 0);
    }

    public StatisticsWeekWidget(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public StatisticsWeekWidget(Context context, AttributeSet attrs, int defStyle) {
        ...
        findViewById(R.id.ll_widget_min_coodinary).setVisibility(VISIBLE);// 分坐标
        findViewById(R.id.ll_widget_hour_coodinary).setVisibility(INVISIBLE);// 时坐标
    ...
    public void setStatistics(final List<StatisticsBean> beans) {
        rcvStatistics.post(new Runnable() {
            ...
            public void run() {
                ...
                StatisticsWeekAdapter adapter = new StatisticsWeekAdapter(context, beans, rcvStatistics);
                adapter.setOnClickItemListener(new StatisticsWeekAdapter.OnClickItemListener() {
                    @Override
     ...
     */
    public void setGoal(@IntRange(from = 0, to = 60) final int mins) {
    ...
    private float getGoalCurrentMarginTop() {
        ...
        float recWidthPercent = Float.valueOf(context.getString(R.string.week_rec_widthPercent).split("%")[0]) / 100;
    ...
    private float getGoalCurrentMarginBottom() {
        ...
        float tvSizePercent = Float.valueOf(context.getString(R.string.week_tv_sizePercent).split("%")[0]) / 100;
        float recWidthPercent = Float.valueOf(context.getString(R.string.week_rec_widthPercent).split("%")[0]) / 100;
        float rlWidthPercent = Float.valueOf(context.getString(R.string.week_rlBlue_widthPercent).split("%")[0]) * 1f / 100;
        float rlBlueMarginBottomPercent = Float.valueOf(context.getString(R.string.week_rlBlue_marginBottomPercent).split("%")[0]) * 1f / 100;
